...
function vless(proxy: Proxy): string {
  ...
  if (
    ['splithttp', 'xhttp'].includes(proxy.network || '') &&
    proxy.mode !== undefined
  ) {
  ...
  const produce = (proxy: Proxy): string => {
    ...
    switch (proxy.type) {
      ...
      case 'ssr':
        ...
        result += `?remarks=${Base64.encode(proxy.name)}${
          ...
          proxy['protocol-param']
            ? '&protocolparam=' + Base64.encode(proxy['protocol-param'])
            : ''
        }`
      ...
      case 'vmess':
        ...
        // obfs
        if (proxy.network) {
          const vmessTransportPath = proxy[`${proxy.network}-opts`]?.path
          const vmessTransportHost =
            proxy[`${proxy.network}-opts`]?.headers?.Host

          if (['grpc'].includes(proxy.network)) {
            vmessResult.path =
              proxy[`${proxy.network}-opts`]?.['grpc-service-name']
            // https://github.com/XTLS/Xray-core/issues/91
            vmessResult.type =
              proxy[`${proxy.network}-opts`]?.['_grpc-type'] || 'gun'
            vmessResult.host =
            ...
            // https://github.com/XTLS/Xray-core/issues/91
            vmessResult.type =
              proxy[`${proxy.network}-opts`]?.[`_${proxy.network}-type`] ||
              'none'
            vmessResult.host =
              proxy[`${proxy.network}-opts`]?.[`_${proxy.network}-host`]
            vmessResult.path =
              proxy[`${proxy.network}-opts`]?.[`_${proxy.network}-path`]
          } else {
      ...
      case 'trojan':
        ...
        if (proxy.network) {
          ...
          const trojanTransportPath = proxy[`${proxy.network}-opts`]?.path
        ...
        if (proxy.obfs) {
          ...
          if (proxy['obfs-password']) {
            hysteria2params.push(
              `obfs-password=${encodeURIComponent(proxy['obfs-password'])}`
            )
          }
        }
        if (proxy.sni) {
      ...
      case 'hysteria':
        ...
        result = `hysteria://${proxy.server}:${
          proxy.port
        }?${hysteriaParams.join('&')}#${encodeURIComponent(proxy.name)}`
        break
          ...
          Object.keys(proxy).forEach((key) => {
            if (
              ...
              if (['alpn'].includes(key)) {
                ...
                tuicParams.push(
                  `${i.replace(/-/g, '_')}=${encodeURIComponent(proxy[key])}`
                )
              }
            }
          })
      ...
      case 'anytls':
        ...
        if (proxy['client-fingerprint']) {
          anytlsParams.push(
            `fp=${encodeURIComponent(proxy['client-fingerprint'])}`
          )
        }
        ...
        if (proxy.alpn && Array.isArray(proxy.alpn)) {
          anytlsParams.push(
            `alpn=${proxy.alpn.map(encodeURIComponent).join(',')}`
          )
        }
