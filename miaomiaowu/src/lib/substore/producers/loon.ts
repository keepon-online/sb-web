...
interface Proxy {
  ...
  network?: string
  'ws-opts'?: Record<string, any>
  'http-opts'?: Record<string, any>
  'h2-opts'?: Record<string, any>
  tls?: boolean
  'skip-cert-verify'?: boolean
  sni?: string
  'tls-fingerprint'?: string
  'tls-pubkey-sha256'?: string
  tfo?: boolean
  'block-quic'?: string
  udp?: boolean
  'ip-version'?: string
  ...
  username?: string
  flow?: string
  'reality-opts'?: Record<string, any>
...
interface Producer {
  produce: (proxy: Proxy, type?: string, opts?: Record<string, any>) => string
}

export default function Loon_Producer(): Producer {
  const produce = (
    proxy: Proxy,
    _type?: string,
  ...
  if (isPresent(proxy, 'plugin')) {
    if (proxy.plugin === 'obfs') {
      if (proxy['plugin-opts']?.mode && proxy.cipher!.startsWith('2022-')) {
        throw new Error(`${proxy.cipher} ${proxy.plugin} is not supported`)
      }
...
function shadowsocksr(proxy: Proxy): string {
  const result = new Result(proxy)
  result.append(
...
function trojan(proxy: Proxy): string {
  ...
  if (isPresent(proxy, 'network')) {
    if (proxy.network === 'ws') {
      ...
      result.appendIfPresent(`,path=${proxy['ws-opts']?.path}`, 'ws-opts.path')
      result.appendIfPresent(
...
function vmess(proxy: Proxy, _includeUnsupported?: boolean): string {
...
function vless(proxy: Proxy, _includeUnsupported?: boolean): string {
  let isXtls = false
  const isReality = !!proxy['reality-opts']
