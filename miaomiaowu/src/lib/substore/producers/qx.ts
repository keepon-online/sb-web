import { isPresent, Result } from '@/lib/substore/producers/utils'
...
interface Proxy {
  ...
  'tls-alpn'?: string
  'tls-no-session-ticket'?: boolean
  'tls-no-session-reuse'?: boolean
  tfo?: boolean
  udp?: boolean
  'test-url'?: string
  protocol?: string
...
export default function QX_Producer(): Producer {
  const produce = (
    proxy: Proxy,
    _type?: string,
...
function needTls(proxy: Proxy): boolean {
  return proxy.tls!
}
