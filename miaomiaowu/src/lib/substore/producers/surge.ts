...
function shadowsocks(proxy: Proxy, _includeUnsupportedProxy?: boolean): string {
  ...
  if (isPresent(proxy, 'shadow-tls-password')) {
    ...
    ['shadow-tls'].includes(proxy.plugin || '') &&
    proxy['plugin-opts']
  ) {
...
function snell(proxy: Proxy): string {
  ...
  // obfs
  result.appendIfPresent(`,obfs=${proxy['obfs-opts']?.mode}`, 'obfs-opts.mode')
  result.appendIfPresent(
  ...
  if (isPresent(proxy, 'network')) {
    if (proxy.network === 'ws') {
      ...
      if (includeUnsupportedProxy && ['http'].includes(proxy.network || '')) {
        toast(`Include Unsupported Proxy: network ${proxy.network} -> tcp`)
      } else {
