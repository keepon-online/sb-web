...
interface Proxy {
  ...
  'ws-opts'?: {
    path?: string
    headers?: Record<string, string | string[]>
    'max-early-data'?: number | string
    'early-data-header-name'?: string
    'v2ray-http-upgrade'?: boolean
  }
  'ws-headers'?: Record<string, string | string[]>
  'ws-path'?: string
  'http-opts'?: {
    method?: string
    path?: string | string[]
  ...
  smux?: {
    enabled: boolean
    protocol?: string
    'max-connections'?: number | string
    'max-streams'?: number | string
    'min-streams'?: number | string
    padding?: boolean
    'brutal-opts'?: {
      up?: number | string
      down?: number | string
  ...
  'recv-window'?: number
  recv_window?: number
  disable_mtu_discovery?: boolean | number
...
const ipVersionParser = (proxy: Proxy, parsedProxy: ParsedProxy): void => {
  const strategy = ipVersions[proxy['ip-version'] as IpVersionKey]
...
const detourParser = (proxy: Proxy, parsedProxy: ParsedProxy): void => {
...
const smuxParser = (smux: Proxy['smux'], proxy: ParsedProxy): void => {
...
const wsParser = (proxy: Proxy, parsedProxy: ParsedProxy): void => {
  const transport: Transport = { type: 'ws', headers: {} }
  if (proxy['ws-opts']) {
    const {
      path: wsPath = '',
      headers: wsHeaders = {},
  ...
  if (parsedProxy.tls?.insecure)
    parsedProxy.tls.server_name = (transport.headers!.Host as string[])[0]
  if (proxy['ws-opts'] && proxy['ws-opts']['v2ray-http-upgrade']) {
    transport.type = 'httpupgrade'
    if (transport.headers?.Host) {
      transport.host = (transport.headers.Host as string[])[0]
...
const h2Parser = (proxy: Proxy, parsedProxy: ParsedProxy): void => {
...
const grpcParser = (proxy: Proxy, parsedProxy: ParsedProxy): void => {
...
const sshParser = (proxy: Proxy = {} as Proxy): ParsedProxy => {
...
const httpParser = (proxy: Proxy = {} as Proxy): ParsedProxy => {
  const parsedProxy: ParsedProxy = {
    tag: proxy.name,
    type: 'http',
    server: proxy.server,
...
const socks5Parser = (proxy: Proxy = {} as Proxy): ParsedProxy => {
  const parsedProxy: ParsedProxy = {
    tag: proxy.name,
    type: 'socks',
    server: proxy.server,
...
const shadowTLSParser = (
...
): { type: string; ssPart: ParsedProxy; stPart: ParsedProxy } => {
  const ssPart: ParsedProxy = {
    tag: proxy.name,
    type: 'shadowsocks',
    method: proxy.cipher,
    password: proxy.password,
  ...
  const stPart: ParsedProxy = {
    ...
    password: proxy['plugin-opts']!.password as string,
    tls: {
      enabled: true,
      server_name: proxy['plugin-opts']!.host as string,
      utls: {
        enabled: true,
        fingerprint: proxy['client-fingerprint'],
      },
...
const ssParser = (proxy: Proxy = {} as Proxy): ParsedProxy => {
  const parsedProxy: ParsedProxy = {
    tag: proxy.name,
    type: 'shadowsocks',
    server: proxy.server,
...
const vmessParser = (proxy: Proxy = {} as Proxy): ParsedProxy => {
  const parsedProxy: ParsedProxy = {
    tag: proxy.name,
    type: 'vmess',
    server: proxy.server,
...
const vlessParser = (proxy: Proxy = {} as Proxy): ParsedProxy => {
  const parsedProxy: ParsedProxy = {
    tag: proxy.name,
    type: 'vless',
    server: proxy.server,
...
const trojanParser = (proxy: Proxy = {} as Proxy): ParsedProxy => {
  const parsedProxy: ParsedProxy = {
    tag: proxy.name,
    type: 'trojan',
    server: proxy.server,
...
const hysteriaParser = (proxy: Proxy = {} as Proxy): ParsedProxy => {
  const parsedProxy: ParsedProxy = {
    tag: proxy.name,
    type: 'hysteria',
    server: proxy.server,
...
const hysteria2Parser = (proxy: Proxy = {} as Proxy): ParsedProxy => {
  const parsedProxy: ParsedProxy = {
    tag: proxy.name,
    type: 'hysteria2',
    server: proxy.server,
...
const tuic5Parser = (proxy: Proxy = {} as Proxy): ParsedProxy => {
  const parsedProxy: ParsedProxy = {
    tag: proxy.name,
    type: 'tuic',
    server: proxy.server,
...
const anytlsParser = (proxy: Proxy = {} as Proxy): ParsedProxy => {
  const parsedProxy: ParsedProxy = {
    tag: proxy.name,
    type: 'anytls',
    server: proxy.server,
  ...
  if (/^\d+$/.test(proxy['idle-session-check-interval'] as string))
    parsedProxy.idle_session_check_interval = `${proxy['idle-session-check-interval']}s`
  if (/^\d+$/.test(proxy['idle-session-timeout'] as string))
    parsedProxy.idle_session_timeout = `${proxy['idle-session-timeout']}s`
  if (/^\d+$/.test(proxy['min-idle-session'] as string))
    parsedProxy.min_idle_session = parseInt(`${proxy['min-idle-session']}`, 10)
  networkParser(proxy, parsedProxy)
  detourParser(proxy, parsedProxy)
  tlsParser(proxy, parsedProxy)
  ipVersionParser(proxy, parsedProxy)
...
export default function singbox_Producer(): Producer {
  const type = 'ALL'
  const produce = (
    proxies: Proxy[],
    type: string,
