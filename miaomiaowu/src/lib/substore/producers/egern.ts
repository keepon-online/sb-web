...
export default function Egern_Producer(): Producer {
  const type = 'ALL'
  const produce = (
    proxies: Proxy[],
    type?: string,
    ...
    const list = proxies
      .filter((proxy) => {
        if (
          ...
          (proxy.type === 'ss' &&
            ((proxy.plugin === 'obfs' &&
              !['http', 'tls'].includes(proxy['plugin-opts']?.mode)) ||
              ![
      ...
      .map((proxy) => {
        ...
        if (proxy.type === 'http') {
          ...
          if (proxy.network === 'ws') {
            ...
            proxy.transport = {
              http1: {
                ...
                headers: {
                  Host: Array.isArray(proxy['http-opts']?.headers?.Host)
            ...
            proxy.transport = {
              http2: {
                ...
                path: Array.isArray(proxy['h2-opts']?.path)
                  ...
                  : proxy['h2-opts']?.path,
                headers: {
                  Host: Array.isArray(proxy['h2-opts']?.headers?.Host)
          ...
          if (proxy.network === 'ws') {
            ...
            let reality: Record<string, any> | undefined
            if (
        ...
        if (
          ['http', 'socks5', 'ss', 'trojan', 'vless', 'vmess'].includes(
            original.type
          )
        ) {
        ...
        if (
          ...
          proxy.shadow_tls &&
          (original['udp-port'] ?? 0) > 0 &&
          (original['udp-port'] ?? 0) <= 65535
        ) {
        ...
        if (proxy.transport) {
          for (const key in proxy.transport) {
            if (
              ...
              Object.values(proxy.transport[key]).every((v) => v == null)
            ) {
