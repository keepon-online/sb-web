...
function SubscribeFilesPage() {
  ...
  const [isNodesDraftRecoveryOpen, setIsNodesDraftRecoveryOpen] =
    useState(false)
  ...
  const { data: allNodesData } = useQuery({
    ...
    enabled: Boolean(
      auth.accessToken && (isExternalSubsExpanded || hasTemplateBindings)
    ),
  })
  ...
  const uploadMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['user-subscriptions'] })
      toast.success(
        uploadForm.overwrite_id > 0 ? '文件覆盖成功' : '文件上传成功'
      )
  ...
  const updateMetadataMutation = useMutation({
    mutationFn: async (payload: {
      id: number
      data: Partial<typeof metadataForm>
    }) => {
        ...
        if (editingNodesFile && editNodesDialogOpen) {
          ...
          if (draftJson) {
            try {
              ...
              if (
                JSON.stringify(normalize(draft.proxyGroups)) !==
  ...
  useEffect(() => {
    ...
    if (
      JSON.stringify(normalize(proxyGroups)) ===
      JSON.stringify(
        normalize(JSON.parse(proxyGroupsOriginalRef.current || '[]'))
      )
    )
      return
  ...
  const handleEditMetadata = (file: SubscribeFile) => {
    ...
    setMetadataForm({
      ...
      traffic_limit:
        file.traffic_limit != null ? String(file.traffic_limit) : '',
    })
    ...
    updateMetadataMutation.mutate({
      ...
      data: {
        ...
        expire_at: metadataForm.expire
          ...
          : '',
        traffic_limit: metadataForm.traffic_limit
          ? parseFloat(metadataForm.traffic_limit)
          : null,
      },
    })
  }
  ...
  const handleApplyReplacement = async () => {
    try {
      ...
      queryClient.setQueryData(
        ['nodes-config-content', editingNodesFile?.filename],
        {
          content: finalConfig,
        }
      )
  ...
  const availableNodes = useMemo(() => {
    ...
    proxyGroups,
    showAllNodes,
    userConfigQuery.data?.node_order,
  ])
  ...
  return (
    <main className='mx-auto w-full max-w-7xl px-4 py-8 pt-24 sm:px-6'>
                ...
                >
                  ...
                  <DialogContent>
                    ...
                    <div className='space-y-4 py-4'>
                      ...
                      <div className='flex items-center justify-between'>
                        <Label htmlFor='upload-raw-output'>
                          这不是 Clash 配置
                        </Label>
                        <Switch
                      ...
                      <div className='space-y-2'>
                        ...
                        <Input
                          ...
                          type='file'
                          accept={
                            uploadForm.raw_output ? undefined : '.yaml,.yml'
                          }
                          onChange={(e) =>
                  ...
                  [
                    {
                      ...
                      cell: (file) => (
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <span className='block max-w-[16ch] truncate font-medium'>
                              {file.name}
                            </span>
                          </TooltipTrigger>
                          ...
                          <Switch
                            checked={
                              !!file.template_filename
                                ? false
                                : file.auto_sync_custom_rules || false
                            }
                            onCheckedChange={(checked) =>
                          ...
                          >
                            <PopoverTrigger asChild>
                              <Button
                                variant='ghost'
                                size='sm'
                                className='h-8 max-w-[80px] truncate px-2 font-mono text-xs'
                              >
                                {code ? (
                                  <Tooltip>
                                    <TooltipTrigger asChild>
                      ...
                      ? ([
                          {
                            ...
                            cell: (file: SubscribeFile) => {
                              const selectedTemplate = v3Templates.find(
                                (t) => t.filename === file.template_filename
                              )
                              return (
                                <Popover>
                                  <PopoverTrigger asChild>
                      ...
                      : []),
                    {
                      ...
                      cell: (file) => {
                        ...
                        const progressBar = (() => {
                          if (
                            !displayTraffic ||
                            displayTraffic.limit_gb === 0
                          ) {
                          }
                          const percentage = Math.min(
                            (displayTraffic.used_gb / displayTraffic.limit_gb) *
                              100,
                            100
                          )
                          const remainingGB = Math.max(
                            displayTraffic.limit_gb - displayTraffic.used_gb,
                            0
                          )
                          return (
                            <Tooltip>
                              <TooltipTrigger asChild>
                                <div className='w-20 cursor-pointer space-y-1'>
                                  <Progress
                                    value={percentage}
                                    className='h-2'
                                  />
                            ...
                            >
                              <div className='space-y-1'>
                                ...
                                <Input
                                  ...
                                  step='0.01'
                                  defaultValue={
                                    file.traffic_limit != null
                                      ? String(file.traffic_limit)
                                      : ''
                                  }
                                  ref={(el) => {
                                    trafficInputRef = el
                    ...
                    <TooltipTrigger asChild>
                      <Button
                        ...
                        onClick={() =>
                          navigate({
                            to: '/nodes',
                            search: { action: 'import-subscription' },
                          })
                        }
                      >
                        <Plus className='h-4 w-4' />
                  ...
                  <Button variant='outline' className='w-full justify-between'>
                    <span className='truncate'>
                      {metadataForm.template_filename
                        ? v3Templates.find(
                            (t) => t.filename === metadataForm.template_filename
                          )?.name || metadataForm.template_filename
                        : '不绑定模板'}
                    </span>
                ...
                >
                  <div className='flex max-h-[300px] flex-col overflow-y-auto'>
                    ...
                    >
                      ...
                      <span
                        className={
                          !metadataForm.template_filename ? '' : 'ml-6'
                        }
                      >
                        不绑定模板
                      </span>
                    ...
                    {v3Templates.map((template) => (
                      ...
                      >
                        ...
                        <span
                          className={
                            metadataForm.template_filename === template.filename
                              ? ''
                              : 'ml-6'
                          }
                        >
                ...
                <Popover>
                  ...
                  <PopoverContent className='w-[300px] p-1' align='start'>
                    <div className='flex max-h-[300px] flex-col overflow-y-auto'>
                      ...
                      {allNodeTags.map((tag) => {
                        const isSelected =
                          metadataForm.selected_tags.includes(tag)
                        return (
                          <Button
                            key={tag}
                            variant='ghost'
                            ...
                            onClick={() => {
                              const newTags = isSelected
                                ? metadataForm.selected_tags.filter(
                                    (t) => t !== tag
                                  )
      ...
      {/* 编辑节点草稿恢复对话框 */}
      <AlertDialog
        open={isNodesDraftRecoveryOpen}
        onOpenChange={setIsNodesDraftRecoveryOpen}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel onClick={handleDiscardNodesDraft}>
              放弃
            </AlertDialogCancel>
            <AlertDialogAction onClick={handleRecoverNodesDraft}>
              恢复
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      ...
      >
        <DialogContent className='max-w-md'>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleApplyReplacement}>应用替换</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
          ...
          <div className='space-y-4'>
            ...
            <div className='space-y-2'>
              ...
              <Select
                ...
                onValueChange={(
                  value: 'download' | 'upload' | 'both' | 'none'
                ) =>
                  setEditExternalSubForm((prev) => ({
                    ...prev,
                    traffic_mode: value,
